...

{
    ...
    ISO8211Assert(nFieldOffset <= static_cast<int>(osData.size()));
    nReadBytes = ISO8211FRead(osData.data() + nFieldOffset, 1,
                           osData.size() - nFieldOffset, poModule->GetFP());
    if (nReadBytes != osData.size() - nFieldOffset && nReadBytes == 0 &&
        ISO8211FEof(poModule->GetFP()))
    {
        return false;
    }
    else if (nReadBytes != osData.size() - nFieldOffset)
    {
        ISO8211Error(ISO8211ErrorFailure, ISO8211FileIO, "Data record is short on DDF file.");
...

{
    ...
    /*      Write the leader.                                               */
    /* -------------------------------------------------------------------- */
    int bRet = ISO8211FWrite(szLeader, nLeaderSize, 1, poModule->GetFP()) > 0;

    /* -------------------------------------------------------------------- */
    /*      Write the remainder of the record.                              */
    /* -------------------------------------------------------------------- */
    bRet &= ISO8211FWrite(osData.data(), osData.size(), 1, poModule->GetFP()) > 0;
...

{
    ...
    nReadBytes = static_cast<int>(
        ISO8211FRead(achLeader, 1, nLeaderSize, poModule->GetFP()));
    if (nReadBytes == 0 && ISO8211FEof(poModule->GetFP()))
    {
        nFieldOffset = -1;
        return false;
    }
    ...
    {
        ISO8211Error(ISO8211ErrorFailure, ISO8211FileIO, "Leader is short on DDF file.");
        nFieldOffset = -1;
        return false;
    }
    ...
    {
        ISO8211Error(
            ISO8211ErrorFailure, ISO8211FileIO,
    ...
    if (_recLength != 0)
    {
        ...
        if (ISO8211FRead(osData.data(), 1, osData.size(), poModule->GetFP()) !=
            osData.size())
        {
                ...
                osData[osData.size() - 2] != DDF_FIELD_TERMINATOR))
        {
            ...
            if (ISO8211FRead(&(osData.back()), 1, 1, poModule->GetFP()) != 1)
            {
                ISO8211Error(ISO8211ErrorFailure, ISO8211FileIO,
                         "Data record is short on DDF file.");
                nFieldOffset = -1;
        ...
        {
            ISO8211Error(ISO8211ErrorFailure, ISO8211AssertionFailed,
        ...
        {
            ISO8211Error(ISO8211ErrorFailure, ISO8211FileIO, "Invalid entry width = %d",
                     nFieldEntryWidth);
            nFieldOffset = -1;
            return false;
        }
    ...
    else
    {
        ...
        if (nFieldEntryWidth == 0)
        {
            ISO8211Error(ISO8211ErrorFailure, ISO8211OutOfMemory,
        ...
        do
        {
            // read an Entry:
            if (nFieldEntryWidth !=
                static_cast<int>(ISO8211FRead(osEntry.data(), 1, nFieldEntryWidth,
                                           poModule->GetFP())))
            {
                ISO8211Error(ISO8211ErrorFailure, ISO8211FileIO,
                         "Data record is short on DDF file.");
        ...
        ISO8211File *fp = poModule->GetFP();
        ISO8211Offset pos = ISO8211FTell(fp) - rewindSize;
        if (ISO8211FSeek(fp, pos, SEEK_SET) < 0)
            return false;
        ...
        for (i = 0; i < nFieldCount; i++)
        {
            ...
            // read an Entry:
            if (nFieldLength !=
                static_cast<int>(ISO8211FRead(osEntry.data(), 1, nFieldLength,
                                           poModule->GetFP())))
            {
                ISO8211Error(ISO8211ErrorFailure, ISO8211FileIO,
                         "Data record is short on DDF file.");
...

{
    for (const auto &poField : apoFields)
    {
        ...
        if (poFieldDefn && ISO8211Equal(poFieldDefn->GetName(), pszName))
        {
            if (iFieldIndex == 0)
                return poField.get();
            else
