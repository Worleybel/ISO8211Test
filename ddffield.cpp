...
bool DDFField::InitializeParts()
{
    ...
    ISO8211Assert(bCreateParts || apoFieldParts.size() == nDefnPartsCount);

    int iOffset = 0;
    const auto &oParts = poDefn->GetParts();
    for (size_t iPart = 0; iPart < oParts.size(); ++iPart)
    {
        const auto &poFieldDefnPart = oParts[iPart];
