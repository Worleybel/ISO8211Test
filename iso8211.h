...
int DDFScanVariable(const char *pszString, int nMaxChars,
                            int nDelimChar);
std::string DDFFetchVariable(const char *pszString, int nMaxChars,
...
class DDFModule
{
  public:
    ...
    void Rewind(ISO8211Offset nOffset = static_cast<ISO8211Offset>(-1));
  ...
  private:
    ISO8211File *fpDDF = nullptr;
    bool bReadOnly = false;
    ISO8211Offset nFirstRecordOffset = 0;
...
class DDFFieldDefn
{
  ...
  private:
    ...
    DDFFieldDefn(const DDFFieldDefn &) = delete;
    DDFFieldDefn &operator=(const DDFFieldDefn &) = delete;
};
