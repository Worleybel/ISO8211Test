...

{
    ...
    if (osFormatString.size() >= 2 && osFormatString[1] == '(')
    {
        ...
        if (nFormatWidth < 0)
        {
            ISO8211Error(ISO8211ErrorFailure, ISO8211AppDefined, "Format width %s is invalid.",
...

{
    if (!bIsVariable)
    {
        ...
        {
            ISO8211Error(ISO8211ErrorWarning, ISO8211AppDefined,
    ...
    else
    {
        ...
        //        if( !bAsciiField )
        //            ISO8211Debug( "ISO8211", "Non-ASCII field detected." );
...

{
    switch (osFormatString[0])
    {
        ...
        case 'b':
        {
            ...
            // word aligned.
            if (ISO8211NeedsByteSwap(osFormatString[0]))
            {
            ...
            switch (eBinaryFormat)
            {
                case UInt:
                    if (nFormatWidth == 1)
                        ...
                        return *(static_cast<std::uint16_t *>(pabyData));
                    else if (nFormatWidth == 4)
                        return *(static_cast<std::uint32_t *>(pabyData));
                    else
                    {
                        // ISO8211Assert( false );
                        return 0.0;
                ...
                case SInt:
                    if (nFormatWidth == 1)
                        ...
                        return *(static_cast<std::int16_t *>(pabyData));
                    else if (nFormatWidth == 4)
                        return *(static_cast<std::int32_t *>(pabyData));
                    else
                    {
                        // ISO8211Assert( false );
                        return 0.0;
...

{
    switch (osFormatString[0])
    {
        ...
        case 'b':
        {
            ...
            {
                ISO8211Error(
                    ISO8211ErrorWarning, ISO8211AppDefined,
                ...
                case UInt:
                    if (nFormatWidth == 4)
                        return static_cast<int>(
                            *(static_cast<std::uint32_t *>(pabyData)));
...

{
    ...
    {
        int nBytes = 0;
        const std::uint8_t *pabyBString = reinterpret_cast<const std::uint8_t *>(
    ...
    else
    {
        ...
        else if (GetBinaryFormat() == FloatReal)
        {
            if (nFormatWidth == 8)
            {
                if (osFormatString[0] == 'B')
                {
                    ISO8211ToBigEndian(&dfNewValue, sizeof(dfNewValue));
                }
                else
                {
                    ISO8211ToLittleEndian(&dfNewValue, sizeof(dfNewValue));
                }
                memcpy(pachData, &dfNewValue, sizeof(dfNewValue));
            }
            else
            {
                ISO8211Assert(nFormatWidth == 4);
                float f = static_cast<float>(dfNewValue);
                if (osFormatString[0] == 'B')
                {
                    ISO8211ToBigEndian(&f, sizeof(f));
                }
                else
                {
                    ISO8211ToLittleEndian(&f, sizeof(f));
                }
