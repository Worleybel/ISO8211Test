...

{
    ...
    ISO8211Assert(iFDOffset >= 6 && iFDOffset <= 9);
    ...
    *ppachData = static_cast<char *>(std::malloc(*pnLength + 1));
...

{
    if (_data_struct_code == dsc_concatenated)
    {
            ...
            _formatControls.front() == '(' && _formatControls.back() == ')')
        {
            ...
            for (size_t i = 0; i < aosPartDescr.size(); ++i)
            {
                ...
                {
                    ISO8211Error(ISO8211ErrorFailure, ISO8211NotSupported,
    ...
    /* -------------------------------------------------------------------- */
    const auto aosSubfieldNames = ISO8211Split(pszSublist, '!');

    /* -------------------------------------------------------------------- */
    /*      minimally initialize the subfields.  More will be done later.   */
    /* -------------------------------------------------------------------- */
    for (const std::string &osSubfieldName : aosSubfieldNames)
    {
        auto poSFDefn = std::make_unique<DDFSubfieldDefn>();

        poSFDefn->SetName(osSubfieldName.c_str());
...

{
    ...
    {
        ISO8211Error(ISO8211ErrorWarning, static_cast<int>(ISO8211DiscardedFormat),
    ...
    /* -------------------------------------------------------------------- */
    const auto aosFormatItems = ISO8211Split(osFormatList.c_str(), ',');
    ...
    {
        const char *pszPastPrefix = aosFormatItems[iFormatItem].c_str();
    ...
    for (auto &poSubfield : apoSubfields)
    {
        ...
        else
        {
            ...
            {
                ISO8211Error(ISO8211ErrorWarning,
                         static_cast<int>(ISO8211DiscardedFormat),
                         "Invalid format controls for `%s': %s", osTag.c_str(),
                         _formatControls.c_str());
                return false;
            }
...

{
    for (const auto &poSubfield : apoSubfields)
    {
        if (ISO8211Equal(poSubfield->GetName(), pszMnemonic))
...

{
    if (!apoFieldParts.empty())
    {
        ...
        char *pabyRet = static_cast<char *>(std::malloc(osData.size()));
    ...
    /*      Allocate buffer.                                                */
    /* -------------------------------------------------------------------- */
    char *pachData = static_cast<char *>(std::malloc(nTotalSize));
    ...
    ISO8211Assert(nOffset == nTotalSize);
